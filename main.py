...
class IkeaSpider(scrapy.Spider):
    ...
    def parse(self, response):
        for room in response.css('a.pub__card'):
            ...
            yield scrapy.Request(room_url, callback=self.parse_category, meta={'depth': 0, 'room_name': room_name})
    ...
    def parse_category(self, response):
        ...
        for category in response.css('.vn-link'):
            ...
            yield scrapy.Request(category_url, callback=self.pre_parse_product, meta={'depth': depth, 'room_name': response.meta.get('room_name'), 'category_name': category.css('.vn__nav__title::text').get()})
    ...
    def pre_parse_product(self, response):
        ...
        if amount_with_text is None:
            if depth < 3:
                ...
                yield scrapy.Request(response.url, callback=self.parse_category, meta={'depth': depth + 1, 'room_name': response.meta.get('room_name'), 'category_name': response.meta.get('category_name')})
            else:
            ...
            if int(amount) > 0:
                yield scrapy.Request(
                    ...
                    callback=self.parse_product, meta={'room_name': response.meta.get('room_name'), 'category_name': response.meta.get('category_name')})
